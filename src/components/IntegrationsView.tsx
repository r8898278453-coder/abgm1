...
import {
  ...
  publishToMetaSocialApi,
  WhatsAppSendResult,
  MetaSocialPublishResult,
...
export const IntegrationsView: React.FC<IntegrationsViewProps> = ({
  ...
  const [waMode, setWaMode] = useState<'text' | 'template' | 'media'>('text');
  ...
  const [waSelectedTemplate, setWaSelectedTemplate] = useState('lead_instant_quote_v1');
  const [waTemplateParam1, setWaTemplateParam1] = useState('Rajesh Kumar');
  const [waTemplateParam2, setWaTemplateParam2] = useState('IT & Digital Growth Package');
  const [waMediaUrl, setWaMediaUrl] = useState('https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=800');
  const [waMediaCaption, setWaMediaCaption] = useState('Aaditech Growth Solution — Official Brochure');
  const [waSending, setWaSending] = useState(false);
  const [waSendResult, setWaSendResult] = useState<WhatsAppSendResult | null>(null);

  // Meta Social Publisher States
  const [isMetaTestOpen, setIsMetaTestOpen] = useState(false);
  const [metaContent, setMetaContent] = useState('🚀 Boost your local business reach with Aaditech Solution! Verified digital marketing & automation services.');
  const [metaImageUrl, setMetaImageUrl] = useState('https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=800');
  const [metaFbChecked, setMetaFbChecked] = useState(true);
  const [metaIgChecked, setMetaIgChecked] = useState(true);
  const [metaPublishing, setMetaPublishing] = useState(false);
  const [metaResult, setMetaResult] = useState<MetaSocialPublishResult | null>(null);
  ...
  const handleSendWhatsAppTest = async () => {
    ...
    try {
      ...
      if (waMode === 'template') {
        payload.templateName = waSelectedTemplate;
        payload.languageCode = 'en_US';
        payload.templateParams = [waTemplateParam1, waTemplateParam2];
      } else if (waMode === 'media') {
        payload.mediaType = 'image';
        payload.mediaUrl = waMediaUrl;
        payload.caption = waMediaCaption;
      } else {
        payload.message = waTestMessage;
      }

      const res = await sendWhatsAppMessageApi(payload);
      setWaSendResult(res);
    } catch (err: any) {
      setWaSendResult({
        success: false,
  ...
  // Meta Social Direct Publish Handler
  const handlePublishMetaSocial = async () => {
    if (!metaContent.trim()) return;
    setMetaPublishing(true);
    setMetaResult(null);
    try {
      const targetPlatforms: Array<'facebook' | 'instagram'> = [];
      if (metaFbChecked) targetPlatforms.push('facebook');
      if (metaIgChecked) targetPlatforms.push('instagram');

      if (targetPlatforms.length === 0) {
        setMetaResult({
          success: false,
          error: 'Please select at least one platform (Facebook or Instagram)',
        });
        return;
      }

      const res = await publishToMetaSocialApi({
        content: metaContent,
      ...
      setMetaResult({
        success: false,
        error: err?.message || 'Failed to dispatch Meta Social post',
      });
    } finally {
  ...
  return (
    <div className="space-y-6">
            ...
            >
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {item.id === 'meta_social' && (
                    <button
                      onClick={() => setIsMetaTestOpen(true)}
                      className="flex items-center gap-1 text-xs font-bold text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-200/60 px-2.5 py-1 rounded-lg transition"
                    >
      ...
      {isWaTestOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs">
          <div className="bg-white rounded-3xl shadow-xl border border-slate-200 max-w-lg w-full p-6 space-y-4 animate-in fade-in zoom-in duration-150 max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl gap-1 text-xs">
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg font-bold transition ${
                  waMode === 'text' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                Direct Message
              </button>
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg font-bold transition ${
                  waMode === 'template' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                Meta Template
              </button>
              <button
                type="button"
                onClick={() => setWaMode('media')}
                className={`flex-1 py-1.5 rounded-lg font-bold transition ${
                  waMode === 'media' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                Media Attachment
              </button>
            ...
            <div className="space-y-3">
              ...
              {waMode === 'template' && (
                <div className="space-y-2.5 bg-emerald-50/50 p-3 rounded-2xl border border-emerald-100">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Select Meta Template *</label>
                    <select
                    ...
                    >
                      <option value="lead_instant_quote_v1">lead_instant_quote_v1 (Quote & Greeting)</option>
                      <option value="review_request_v1">review_request_v1 (5★ Google Review Request)</option>
                      <option value="festival_offer_alert">festival_offer_alert (Promotional Festival Deal)</option>
                      <option value="appointment_reminder">appointment_reminder (Service Booking Reminder)</option>
                    </select>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      <label className="text-[11px] font-semibold text-slate-600">Param 1 (Client Name)</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1">
                      <label className="text-[11px] font-semibold text-slate-600">Param 2 (Package / Offer)</label>
                      <input
                        type="text"
              ...
              {waMode === 'media' && (
                <div className="space-y-2 bg-slate-50 p-3 rounded-2xl border border-slate-200">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Image / PDF URL *</label>
                    <input
                      type="url"
                      value={waMediaUrl}
                      onChange={(e) => setWaMediaUrl(e.target.value)}
                      placeholder="https://example.com/brochure.jpg"
                      className="w-full text-xs px-3 py-2 rounded-xl border border-slate-300 font-mono bg-white"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <input
                      type="text"
                      value={waMediaCaption}
                      onChange={(e) => setWaMediaCaption(e.target.value)}
                      placeholder="Special festive brochure for you"
                      className="w-full text-xs px-3 py-2 rounded-xl border border-slate-300 bg-white"
                    />
                  </div>
                </div>
            ...
            {/* Inbound Webhook Guidance Accordion */}
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-[11px] text-slate-600 space-y-1">
              <div className="flex items-center justify-between font-bold text-slate-800">
                ...
                <span className="text-[10px] text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">HMAC-SHA256 Signed</span>
              </div>
              <p className="font-mono text-[10px] text-slate-700 break-all bg-white p-1.5 rounded border border-slate-200">
                {window.location.origin}/api/whatsapp/webhook
              </p>
              <div className="text-[10px] text-slate-500">
                Verify Token: <strong className="font-mono text-slate-800">aaditech_bga_whatsapp_verify_token</strong>
              </div>
            </div>
      ...
      {/* Meta Social Live Dispatcher Modal */}
      {isMetaTestOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs">
          <div className="bg-white rounded-3xl shadow-xl border border-slate-200 max-w-lg w-full p-6 space-y-4 animate-in fade-in zoom-in duration-150">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-blue-50 border border-blue-100 flex items-center justify-center text-xl text-blue-600">
                  🌐
                </div>
                <div>
                  <h3 className="font-bold text-slate-900 text-base">Meta Social Direct Dispatcher</h3>
                  <p className="text-xs text-slate-500">Publish live posts directly to Facebook Pages & Instagram Business</p>
                </div>
              </div>
              <button
                onClick={() => {
                  setIsMetaTestOpen(false);
                  setMetaResult(null);
                }}
                className="text-slate-400 hover:text-slate-600 p-1.5 rounded-lg hover:bg-slate-100"
              >
            ...
            <div className="bg-blue-50/70 border border-blue-100 rounded-xl p-3 text-xs text-blue-950 leading-relaxed">
              <strong>Official Graph API v21.0:</strong> Publishes directly to your linked Facebook Page feed and Instagram Business profile with live photo upload containers.
            </div>

            <div className="space-y-3">
              {/* Platform Selector */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700">Target Platforms</label>
                <div className="flex items-center gap-4 text-xs font-medium text-slate-700">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={metaFbChecked}
                      onChange={(e) => setMetaFbChecked(e.target.checked)}
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={metaIgChecked}
                      onChange={(e) => setMetaIgChecked(e.target.checked)}
              ...
              {/* Post Content */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700">Post Content / Caption *</label>
                <textarea
                  rows={3}
                  value={metaContent}
                  onChange={(e) => setMetaContent(e.target.value)}
                  placeholder="What's new with your business today?"
                  className="w-full text-xs p-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-500 bg-white leading-relaxed"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full text-xs px-3 py-2.5 rounded-xl border border-slate-300 font-mono bg-white"
                />
              </div>
            </div>
            ...
            {metaResult && (
              ...
              >
                <div className="flex items-center gap-2 font-bold">
                  ...
                  <span>{metaResult.success ? 'Post Dispatch Successful!' : 'Dispatch Notice'}</span>
                </div>

                <p className="text-[11px] leading-relaxed">{metaResult.message || metaResult.error}</p>

                {metaResult.results && (
                  <div className="text-[10px] space-y-1 font-mono bg-white/80 p-2 rounded border border-slate-200">
                    {metaResult.results.facebook && (
                      <div>
                        <strong>Facebook:</strong> {metaResult.results.facebook.success ? `Post ID: ${metaResult.results.facebook.id}` : `Error: ${metaResult.results.facebook.error}`}
                      </div>
                    )}
                    {metaResult.results.instagram && (
                      <div>
                        <strong>Instagram:</strong> {metaResult.results.instagram.success ? `Media ID: ${metaResult.results.instagram.id}` : `Error: ${metaResult.results.instagram.error}`}
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-100">
              ...
              <button
                type="button"
                onClick={handlePublishMetaSocial}
                disabled={metaPublishing || !metaContent.trim()}
              ...
              >
                {metaPublishing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
                {metaPublishing ? 'Publishing...' : 'Publish to Meta'}
              </button>
            </div>
          </div>
        </div>
